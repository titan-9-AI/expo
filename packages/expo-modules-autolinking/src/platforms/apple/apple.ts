...
/**
 * Merges the modules declared in `expo-module.config.json` with the scanned `@ExpoModule` classes,
 * deduplicated by class name. Config entries keep their declared order (so existing generated
 * providers don't churn); classes found only by the scan are appended in a stable alphabetical
 * order.
 */
function mergeWithScannedModules(
  configModules: ModuleIosConfig[],
  scannedModules: ModuleIosConfig[]
): ModuleIosConfig[] {
  const configClasses = new Set(configModules.map((module) => module.class));
  const scannedOnly = scannedModules
    .filter((module) => !configClasses.has(module.class))
    .sort((a, b) => a.class.localeCompare(b.class));
  return [...configModules, ...scannedOnly];
}
...
export async function resolveModuleAsync(
  ...
  return {
    ...
    modules: mergeWithScannedModules(
      configModules,
